...
class RtSearchModel(models.Model):
    ...
    highest_solo_tier = fields.CharField(
        max_length=50, null=True, description="최고 랭크 티어"
    )
    highest_solo_rank = fields.CharField(
        max_length=5, null=True, description="최고 랭크 디비전"
    )
    highest_solo_lp = fields.IntField(null=True, description="최고 리그 포인트")
    highest_flex_tier = fields.CharField(
        max_length=50, null=True, description="현재 랭크 티어"
    )
    highest_flex_rank = fields.CharField(
        max_length=5, null=True, description="현재 랭크 디비전"
    )
